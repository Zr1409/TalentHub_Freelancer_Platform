...
const ClientProfile = () => {
      ...
      try {
        ...
        if (
          ...
          setProfile({
            ...
            mfaEnabled: clientResponse.data.mfaEnabled || false,
          });
        }
  ...
  useEffect(() => {
    ...
    const calculateProfileCompletion = () => {
      const requiredFields = [
        ...
        {
          name: "Vị trí",
          value: profile.province !== null && profile.ward !== null,
        },
  ...
  const handleCompanySubmit = async () => {
    try {
      ...
      if (hasCompany && company.id) {
        ...
        const { companyImage, companyName, address, phoneContact, industry } = company;
  ...
  const handleAvatarButtonClick = (type: 'user' | 'company') => {
    if (type === 'user' && userFileInputRef.current) {
      userFileInputRef.current.click();
    }
    if (type === 'company' && companyFileInputRef.current) {
  ...
  const handleUserFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const imageUrl = await userService.uploadImage(file, "talenthub_users");
  ...
  const handleCompanyFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Upload ảnh lên storage (folder riêng cho company)
      const imageUrl = await userService.uploadImage(file, "talenthub_companies");

      // Gọi API update logo công ty
      if (company.id) {
        const updateResponse = await clientService.updateImageCompany(company.id, imageUrl);

        if (updateResponse.status === 200) {
          setCompany((prev) => ({ ...prev, companyImage: imageUrl }));
          notification.success({
            message: "Thành công",
        ...
        // Nếu công ty chưa có, chỉ cập nhật state trước
        setCompany((prev) => ({ ...prev, companyImage: imageUrl }));
      }
    } catch (error) {
      ...
      notification.error({
        message: "Lỗi",
        description: "Không thể cập nhật logo công ty. Vui lòng thử lại sau.",
      });
    } finally {
      setUploadingImage(false);
    ...
    <div className="max-w-4xl mx-auto">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
          ...
          <form onSubmit={handleSubmit}>
            <Card className="p-6 mb-6">
                ...
                <div className="flex items-center gap-6 mb-8">
                  <div className="relative">
                    ...
                    <Button
                      ...
                      type="button"
                      onClick={() => handleAvatarButtonClick('user')}
                      disabled={uploadingImage}
                    >
                      {uploadingImage ? (
                  ...
                  <div>
                    ...
                    <div
                      className={`text-sm mt-1 px-3 py-1 rounded-full inline-flex items-center gap-1.5 ${profile.status === "Khoá"
              ...
              <div className="mt-6 flex justify-end">
                <Button className="hover:bg-primary-700" type="submit" disabled={loading || uploadingImage}>
                  {loading ? (
                    <>
          ...
          <Card className="p-6">
            <FadeInWhenVisible>
              ...
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                  ...
                  <div className="relative">                  
                    ...
                    <Button
                      ...
                      className="absolute bottom-0 right-0 rounded-full"
                      type="button"
                      onClick={() => handleAvatarButtonClick('company')}
                      disabled={uploadingImage}
                    >
                      {uploadingImage ? (
                        <Loader2 className="w-4 h-4 animate-spin" />
                ...
                {hasCompany && (
                  <Button
                    className="border-primary-300 hover:text-red-500 hover:text-white hover:border-red-300 hover:bg-red-500"
                    variant="outline"
                    size="sm"
                  ...
                  >
                    {companyLoading ? (
                      ...
                      <>
                        <Trash2 className="h-4 w-4 mr-1 hover:text-red-500" />
                        Xóa
                      </>
                    )}
                  </Button>
              ...
              <FadeInWhenVisible delay={0.3}>
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <Input
                      type="tel"
                      className={`pl-10 ${companyPhoneError
                        ? "border-red-500 focus-visible:ring-red-500"
                        : ""
                        }`}
        ...
        <TabsContent value="security">
          <Security onMfaStatusChange={handleMfaStatusChange} />
        </TabsContent>
