...
const FaceCapture = () => {
  ...
  useEffect(() => {
    ...
    setProgress(Math.min((total / 9) * 100, 100));
  }, [capturedImages]);
  ...
  const handleStartCapture = () => {
    ...
    intervalRef.current = setInterval(async () => {
      ...
      setCapturedImages((prev) => {
        ...
        if (currentImages.length >= 3) return prev;
        ...
        if (updated[currentDir].length === 3) {
          ...
          <div className="flex justify-center gap-4">
            {!isCapturing && !processing ? (
              <Button
                ...
                className="px-6 py-2 bg-blue-600 hover:bg-primary-700 text-white"
              >
                Bắt đầu đăng ký xác thực
              </Button>
            ) : (
              <>
                {isCapturing && (
