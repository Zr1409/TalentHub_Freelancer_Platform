...
const FaceVerification = () => {
  ...
  const faceNotFoundRef = useRef(false); // kiểm soát tần suất hiển thị thông báo
      ...
      try {
        ...
        if (!data.registered) {
          notification.info({
            ...
            description: "Vui lòng đăng ký xác thực khuôn mặt.",
          });
  ...
  useEffect(() => {
    const verifyLoop = async () => {
      ...
      try {
        ...
        if (!res.ok) {
          if (!faceNotFoundRef.current) {
            faceNotFoundRef.current = true;

            notification.error({
              message: "Không tìm thấy khuôn mặt",
              description: data.message || "Vui lòng điều chỉnh camera và thử lại.",
              duration: 3,
            });

            // Sau 5 giây mới cho hiện lại thông báo lỗi
            setTimeout(() => {
              faceNotFoundRef.current = false;
            }, 2000);
          }
          return;
        }
        if (data.success) {
          notification.success({ message: "Xác thực khuôn mặt thành công" });
