...
export function SkillsPage() {
  ...
  const handleDelete = (skillId: string, quantityFreelancerSkill: number, quantityJobSkill: number) => {
    if (quantityFreelancerSkill > 0 || quantityJobSkill > 0) {
      Modal.confirm({
        title: "Xoá kỹ năng",
        content: "Kỹ năng này có freelancer hoặc công việc. Hành động này sẽ xoá toàn bộ các liên quan và không thể thu hồi. Bạn có chắc chắn muốn xoá không?",
        onOk: async () => {
          try {
            await api.delete(`/v1/jobs/skills/${skillId}`);
            setData((prevData) => prevData.filter((skill) => skill.id !== skillId));
            notification.success({ message: "Xoá thành công", description: "Kỹ năng đã bị xoá." });
          } catch (error) {
            notification.error({ message: "Lỗi khi xoá", description: "Không thể xóa kỹ năng này." });
          }
        },
      });
    } else {
      Modal.confirm({
        ...
        onOk: async () => {
          try {
            ...
            notification.error({ message: "Lỗi khi xoá", description: "Không thể xoá kỹ năng này." });
          }
        },
      });
    }
