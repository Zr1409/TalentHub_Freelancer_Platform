...
export function SchoolsPage() {
  ...
  const handleDelete = (schoolId: string, quantityEducation: number) => {
    if (quantityEducation > 0) {
      Modal.confirm({
        title: "Xác nhận xoá",
        content: "Trường này có người dùng. Hành động này sẽ xoá toàn bộ các liên quan và không thể thu hồi. Bạn chắc chắn muốn xoá không?",
        onOk: async () => {
          try {
            await api.delete(`/v1/schools/${schoolId}`);
            setData((prevData) => prevData.filter((school) => school.id !== schoolId));
            notification.success({ message: "Xoá thành công", description: "Trường học đã bị xoá." });
          } catch (error) {
            notification.error({ message: "Lỗi khi xoá", description: "Không thể xoá trường học này." });
          }
        },
      });
    } else {
      Modal.confirm({
        title: "Xác nhận xoá",
        content: "Bạn chắc chắn muốn xoá trường này?",
        onOk: async () => {
          try {
            await api.delete(`/v1/schools/${schoolId}`);
            setData((prevData) => prevData.filter((school) => school.id !== schoolId));
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        >
          <DialogTrigger asChild>
            <Button className="flex items-center gap-2 hover:bg-primary-700">
          ...
          <DialogContent className="sm:max-w-[500px]">
            ...
            <form className="space-y-6" onSubmit={handleSubmit}>
              ...
              <div className="flex justify-end gap-4">
                <Button className="hover:border-green-300 border-primary-300" type="button" variant="outline" onClick={() => setDialogOpen(false)}>
                  Hủy
                </Button>
                <Button className="hover:bg-primary-700" type="submit">
      ...
      <div className="flex items-center mb-4">
        <Label htmlFor="filter" className="mr-2">Tìm kiếm:</Label>
        <Input
          id="filter"
          placeholder="Nhập tên trường"
          value={filterValue}
            ...
            cell: ({ row }) => {
              ...
              return (
                <div className="flex space-x-2">
                  <Button onClick={() => handleEdit(row.original)} variant="outline" className="text-blue-600 hover:border-green-300 border-primary-300">
                    Chỉnh sửa
                  </Button>
                  <Button
                    onClick={() => handleDelete(id, quantity)}
                    variant="outline"
                    className="text-red-600 hover:border-green-300 border-red-300"
                  >
                    Xoá
                  </Button>
                </div>
