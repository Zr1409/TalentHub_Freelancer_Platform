...
export function CategoriesPage() {
  ...
  const handleDelete = (categoryId: string, quantityFreelancer: number, quantityJob: number) => {
    if (quantityFreelancer > 0 || quantityJob > 0) {
      Modal.confirm({
        title: "Xoá danh mục",
        content: "Danh mục này có freelancer hoặc công việc. Hành động này sẽ xoá toàn bộ các liên quan và không thể thu hồi. Bạn có chắc chắn muốn xoá không?",
        onOk: async () => {
          try {
            await api.delete(`/v1/categories/${categoryId}`);
            setData((prevData) => prevData.filter((category) => category.id !== categoryId));
            notification.success({ message: "Xoá thành công", description: "Danh mục đã bị xoá." });
          } catch (error) {
            notification.error({ message: "Lỗi khi xoá", description: "Không thể xoá danh mục này." });
          }
        },
      });
    } else {
