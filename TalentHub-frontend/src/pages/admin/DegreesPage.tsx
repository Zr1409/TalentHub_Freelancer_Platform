...
export function DegreesPage() {
  ...
  const handleDelete = (degreeId: string, quantityEducation: number) => {
    if (quantityEducation > 0) {
      Modal.confirm({
        title: "Xoá bằng cấp",
        content: "Bằng cấp này có người dùng. Hành động này sẽ xoá toàn bộ các liên quan và không thể thu hồi. Bạn có chắc chắn muốn xoá không?",
        onOk: async () => {
          try {
            await api.delete(`/v1/degrees/${degreeId}`);
            setData((prevData) => prevData.filter((degree) => degree.id !== degreeId));
            notification.success({ message: "Xoá thành công", description: "Bằng cấp học đã bị xoá." });
          } catch (error) {
            notification.error({ message: "Lỗi khi xoá", description: "Không thể xoá bằng cấp học này." });
          }
        },
      });
    } else {
