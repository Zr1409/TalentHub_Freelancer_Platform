...
const MessageDropdown = () => {
    const { conversations, setActiveConversationId,  markAsRead} = useMessages();
    ...
    return (
        <DropdownMenu>
            <DropdownMenuTrigger asChild>
                <Button variant="ghost" size="icon" className="relative bg-primary-100">
                    <MessageSquare className="h-5 w-5" />
                    {unreadCount > 0 && (
                        <Badge className="absolute -top-1 -right-1 px-1.5 py-0.5 text-white bg-red-500 text-[10px] min-w-[18px] h-[18px] flex items-center justify-center rounded-full" variant="destructive">
                        ...
                        conversations.slice(0, 5).map((conv) => (
                            <DropdownMenuItem key={conv.id} className="px-4 py-2 focus:bg-primary/10 cursor-pointer">
                                <a
                                    ...
                                    onClick={(e) => {
                                        e.preventDefault();
                                        setActiveConversationId(conv.id); // cập nhật active
                                        markAsRead(conv.id);              // cập nhật đã đọc
                                ...
                                >
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="flex justify-between items-baseline hover:text-black">
                                    ...
                                    {conv.unread > 0 && (
                                        <Badge className="ml-auto bg-red-500  text-primary-foreground text-[10px] md:text-xs rounded-full h-4 min-w-4 md:h-5 md:min-w-5 flex items-center justify-center px-1 ml-1 flex-shrink-0" variant="default">
                                            {conv.unread}
                                        </Badge>
                                    )}
                                </a>
